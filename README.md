...
Очень удобно для <p>, <span>, <h1> и других текстовых элементов.

Работает только для текста, который реально отображается.
...
⚠ Если текста нет → тест падает.

2️⃣ getByRole

Ищет элемент по семантической роли (кнопка, заголовок, текстовое поле и т.д.).

Лучше всего подходит для элементов формы и интерактивных элементов.

Можно уточнить имя (label) через name.
...
Ищет элемент по атрибуту data-testid, который создаётся специально для тестов.

Используется, когда нет уникального текста или роли.
...
⚠ Без data-testid ничего не найдёт.

4️⃣ queryByText / queryByTestId

Отличие от getBy…: не падает, если элемент не найден.

Возвращает null.

Используется, когда нужно проверить, что чего-то нет в компоненте.

const error = screen.queryByText('Error!')
expect(error).toBeNull()

5️⃣ findByText / findByTestId

Асинхронная версия getBy…

Возвращает Promise, ждёт появления элемента (например, после fetch или setTimeout).

const message = await screen.findByText('Loaded')
expect(message).toBeInTheDocument()

6️⃣ container.querySelector / container.querySelectorAll

Позволяет искать элементы по любым CSS-селекторам внутри отрендеренного компонента.

Полезно для сложных структур, где нет текста/роли/testid.

const { container } = render(<Note note={note} />)
const div = container.querySelector('.note')
expect(div).toHaveTextContent('Component testing is done with react-testing-library')

🟢 Итоговая шпаргалка по использованию
Метод	Что ищет	Когда использовать
getByText	По тексту	Стандартный вариант для элементов с уникальным текстом
getByRole	По роли	Для кнопок, input, заголовков
getByTestId	По data-testid	Когда нет уникального текста или роли
queryBy…	По тексту/тестиду	Проверка отсутствия элемента
findBy…	Асинхронно по тексту/тестиду	Когда элемент появляется с задержкой (fetch, setTimeout)
container.querySelector	По CSS-селекторам	Для сложных или специфичных случаев
